...
Отвечай строго на русском языке. Не используй английские слова.
...
Требования:
- transcript: 2–3 предложения на русском.
- explanation: 4–6 предложений на русском простым языком.
- insights: 3 короткие смысловые заметки на русском.
- disclaimer: 1 фраза на русском о том, что это интерпретация сонфикации.
    """.strip()
...
def analyze_sound():
    ...
    try:
        ...
        # Если ответ не на русском, просим перевести JSON.
        joined_text = " ".join([
            structured.get('transcript', ''),
            structured.get('explanation', ''),
            " ".join(structured.get('insights', []) if isinstance(structured.get('insights'), list) else []),
            structured.get('disclaimer', '')
        ])
        if structured and not contains_cyrillic(joined_text):
            try:
                translate_prompt = (
                    "Переведи на русский следующий JSON, сохрани ключи и формат. "
                    "Не добавляй новых полей и не добавляй Markdown. JSON:\\n"
                    f"{json.dumps(structured, ensure_ascii=False)}"
                )
                translate_response = model.generate_content(translate_prompt)
                if translate_response and getattr(translate_response, 'text', None):
                    translated = extract_json(translate_response.text.strip())
                    if translated:
